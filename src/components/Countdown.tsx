  ...
  return (
    <section className="xp-count-section" ref={sectionRef}>
      <div className="xp-bg-wrap" aria-hidden>
        ...
        />
        <div className="xp-bg-tint" />
        <div className="xp-bg-vignette" />
      </div>
      ...
      <div className="xp-count-wrap" data-anim="fade-up">
        ...
        <div className="timer-outer" ref={outerRef}>
          <div className="timer-scale" style={{ transform: `scale(${scale})` }}>
            ...
            >
              {left ? (
                <>
                  <div className="t-block" aria-label={`${left.days} días`}>
                    <span className="t-num">{String(left.days).padStart(2, "0")}</span>
                    <span className="t-lbl">DÍAS</span>
                  </div>
                  <span className="t-colon" aria-hidden>:</span>
                  <div className="t-block" aria-label={`${left.hours} horas`}>
                    <span className="t-num">{String(left.hours).padStart(2, "0")}</span>
                    <span className="t-lbl">HRS</span>
                  </div>
                  <span className="t-colon" aria-hidden>:</span>
                  <div className="t-block" aria-label={`${left.minutes} minutos`}>
                    <span className="t-num">{String(left.minutes).padStart(2, "0")}</span>
                    <span className="t-lbl">MIN</span>
                  </div>
                  <span className="t-colon" aria-hidden>:</span>
                  <div className="t-block" aria-label={`${left.seconds} segundos`}>
                    <span className="t-num">{String(left.seconds).padStart(2, "0")}</span>
                    <span className="t-lbl">SEG</span>
                  </div>
                </>
              ) : (
        ...
        <div className="xp-meta3" data-anim="fade-up" style={{ ["--d" as any]: "260ms" }} data-parallax="-8">
          ...
          <span className="m-dot" />
          <a
            className="m-item link-ghost"
            href="https://www.google.com/maps/search/?api=1&query=Hotel+Los+Tajibos+Santa+Cruz"
